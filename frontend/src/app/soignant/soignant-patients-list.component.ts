    ...
    <div class="bg-white dark:bg-slate-800/80 rounded-2xl border border-slate-200/60 dark:border-slate-700/60 p-6 shadow-soft">
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Suivez les dossiers en cours et l'état de vos patients.</p>
        </div>
        <div class="flex items-center gap-3">
          <div class="relative">
            ...
            <input [(ngModel)]="search" placeholder="Rechercher un patient..."
              class="pl-10 pr-4 py-2.5 w-full lg:w-72 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-sm text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 transition-all" />
          </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
      <div *ngFor="let p of filtered()"
        ...
        <div class="pl-3">
          ...
          <!-- Action -->
          <a [routerLink]="['/soignant/patients', p.idPatient]"
            class="inline-flex items-center gap-1.5 px-4 py-2 text-sm rounded-xl bg-slate-50 dark:bg-slate-700/50 text-teal-600 dark:text-teal-400 font-semibold border border-slate-200/60 dark:border-slate-600/60 hover:bg-teal-600 hover:text-white hover:border-teal-600 hover:shadow-lg hover:shadow-teal-500/20 transition-all duration-200">
